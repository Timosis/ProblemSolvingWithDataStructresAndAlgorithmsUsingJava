...
package main.Dynamic_Programming;
...
public class HouseRobber {
    ...
    public static int robbingHouseTest(int[] nums){
        ...
        dpMemo[0] = nums[0];
        dpMemo[1] = Math.max(nums[0],nums[1]);

        for (int i = 2; i < dpMemo.length ; i++) {
            dpMemo[i] = Math.max(nums[i] + dpMemo[i - 2], dpMemo[i -1]);
        }
        return dpMemo[nums.length - 1];
    }
}
