...
public class CriticalConnectionInNetworkAnotherSolution {
    public List<List<Integer>> criticalConnections(int n, List<List<Integer>> connections){
        ...
        criticalConnectionUtils(graph, -1, 0, timer, visited, results, timeStampAtThatNode);
        return results;
    }

    private void criticalConnectionUtils(List<Integer>[] graph, int parent, int node, int[] timer, boolean[] visited, List<List<Integer>> results, int[] timeStampAtThatNode) {
        visited[node] = true;
        ...
        for(int oneNeighbour:graph[node]){
            ...
            if (!visited[oneNeighbour]) {
                criticalConnectionUtils(graph,node,oneNeighbour,timer,visited,results,timeStampAtThatNode);
            }
